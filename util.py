# import re
# import csv
# import easyocr
# import cv2
# import ast

# # ---------------- OCR setup ----------------
# # Initialize once (set gpu=True if you have CUDA properly installed)
# reader = easyocr.Reader(['en'], gpu=False)

# # ---------------- Heuristics for plate cleanup ----------------
# char_to_num = {'O': '0', 'D': '0', 'Q': '0', 'I': '1', 'L': '1', 'Z': '2', 'S': '5', 'B': '8', 'H': 'N'}
# num_to_char = {'0': 'O', '1': 'I', '2': 'Z', '5': 'S', '8': 'B'}

# def strict_format(text: str) -> bool:
#     # Indian plate format: 9–10 chars like MH12AB1234 or MH12A1234
#     if not (9 <= len(text) <= 10):
#         return False
#     if not (text[0].isalpha() and text[1].isalpha()):
#         return False
#     if not (text[2].isdigit() and text[3].isdigit()):
#         return False
#     if not text[4].isalpha():
#         return False
#     if len(text) == 10 and not text[5].isalpha():
#         return False
#     if len(text) == 9 and not text[5].isdigit():
#         return False
#     if not all(ch.isdigit() for ch in text[-4:]):
#         return False
#     return True

# def clean_and_correct(text: str) -> str:
#     text = re.sub(r'[^A-Z0-9]', '', text.upper())
#     if len(text) > 10:
#         text = text[:10]

#     text_list = list(text)
#     for idx, ch in enumerate(text_list):
#         # letters expected at indices 0,1,4,(5 if len==10); digits elsewhere
#         if idx in [0, 1, 4, 5] and idx < len(text_list):
#             if ch.isdigit():
#                 text_list[idx] = num_to_char.get(ch, ch)
#         else:
#             if ch.isalpha():
#                 text_list[idx] = char_to_num.get(ch, ch)
#     return ''.join(text_list)

# def read_license_plate(lp_img):
#     detections = reader.readtext(lp_img)
#     if not detections:
#         return None, None
#     merged = "".join([det[1] for det in detections]).upper()
#     scores = [det[2] for det in detections if len(det) >= 3]
#     mean_score = sum(scores) / len(scores) if scores else 0.0

#     merged = clean_and_correct(merged)
#     if strict_format(merged):
#         return merged, mean_score
#     return None, None

# def get_car(lp, vehicle_track_ids):
#     x1, y1, x2, y2, score, class_id = lp
#     for xcar1, ycar1, xcar2, ycar2, car_id in vehicle_track_ids:
#         if x1 > xcar1 and y1 > ycar1 and x2 < xcar2 and y2 < ycar2:
#             return xcar1, ycar1, xcar2, ycar2, car_id
#     return -1, -1, -1, -1, -1

# # ---- Levenshtein & similarity
# def _levenshtein(a: str, b: str) -> int:
#     if a == b:
#         return 0
#     if len(a) < len(b):
#         a, b = b, a
#     if len(b) == 0:
#         return len(a)
#     prev = list(range(len(b) + 1))
#     for i, ca in enumerate(a, 1):
#         curr = [i]
#         for j, cb in enumerate(b, 1):
#             ins = prev[j] + 1
#             dele = curr[j - 1] + 1
#             sub = prev[j - 1] + (ca != cb)
#             curr.append(min(ins, dele, sub))
#         prev = curr
#     return prev[-1]

# def _similar_plates(p1: str, p2: str, threshold: float = 0.85) -> bool:
#     if not p1 or not p2:
#         return False
#     if p1 == p2:
#         return True
#     if len(p1) == len(p2) and _levenshtein(p1, p2) <= 1:
#         return True
#     dist = _levenshtein(p1, p2)
#     maxlen = max(len(p1), len(p2))
#     ratio = 1.0 - (dist / maxlen)
#     return ratio >= threshold

# def write_csv(results, output_path, unify_by_plate: bool = True, similarity_threshold: float = 0.85):
#     """
#     CSV columns:
#     frame_nmr, car_id, car_bbox, license_plate_bbox, license_plate_bbox_score, license_number, license_number_score
#     """
#     plate_to_gid = {}
#     sort_to_gid = {}
#     next_gid = 0

#     frame_keys = sorted(results.keys(), key=lambda x: int(x) if str(x).isdigit() else x)

#     with open(output_path, 'w', newline='') as f:
#         f.write('frame_nmr,car_id,car_bbox,license_plate_bbox,license_plate_bbox_score,license_number,license_number_score\n')

#         for frame_nmr in frame_keys:
#             cars = results[frame_nmr]
#             for sort_car_id in cars.keys():
#                 data = cars[sort_car_id]
#                 if 'car' not in data or 'license_plates' not in data:
#                     continue

#                 car_bbox = data['car']['bbox']
#                 lp = data['license_plates']
#                 plate_text = lp.get('text')
#                 plate_score = lp.get('text_score', 0)
#                 lp_bbox = lp.get('bbox')
#                 lp_bbox_score = lp.get('bbox_score', 0)

#                 if unify_by_plate and plate_text and plate_text != 'None':
#                     assigned_gid = None
#                     for known_plate, gid in plate_to_gid.items():
#                         if _similar_plates(plate_text, known_plate, similarity_threshold):
#                             assigned_gid = gid
#                             break
#                     if assigned_gid is None:
#                         assigned_gid = next_gid
#                         plate_to_gid[plate_text] = assigned_gid
#                         next_gid += 1
#                 else:
#                     if sort_car_id not in sort_to_gid:
#                         sort_to_gid[sort_car_id] = next_gid
#                         next_gid += 1
#                     assigned_gid = sort_to_gid[sort_car_id]

#                 f.write('{},{},[{} {} {} {}],[{} {} {} {}],{},{},{}\n'.format(
#                     frame_nmr,
#                     assigned_gid,
#                     *car_bbox,
#                     *lp_bbox,
#                     lp_bbox_score,
#                     plate_text if plate_text is not None else '',
#                     plate_score
#                 ))



# # ---- shared bbox parser
# def parse_bbox(bbox_str):
#     """Convert bbox string to list of floats (handles spaces, commas, signs)."""
#     coords = re.findall(r"[-+]?\d*\.\d+|\d+", str(bbox_str))
#     if len(coords) != 4:
#         raise ValueError(f"Invalid bbox format: {bbox_str}")
#     return list(map(float, coords))



# def parse_bbox(bbox_str):
#     """
#     Parse bounding box string into a list of floats.
#     Example input: "[484.75, 210.82, 1119.80, 830.67]"
#     """
#     if bbox_str is None or bbox_str == "None":
#         return None
#     try:
#         bbox = ast.literal_eval(str(bbox_str))
#         return [float(x) for x in bbox]
#     except Exception:
#         raise ValueError(f"Invalid bbox format: {bbox_str}")







# import re
# import csv
# import json
# import cv2
# import easyocr
# import numpy as np

# # ---------------- OCR setup ----------------
# # Initialize once (set gpu=True if CUDA is available)
# reader = easyocr.Reader(['en'], gpu=False)

# # ---------------- Heuristics for plate cleanup ----------------
# char_to_num = {'O': '0', 'D': '0', 'Q': '0', 'I': '1', 'L': '1', 'Z': '2', 'S': '5', 'B': '8', 'H': 'N'}
# num_to_char = {'0': 'O', '1': 'I', '2': 'Z', '5': 'S', '8': 'B'}

# def strict_format(text: str) -> bool:
#     # Indian plate format: 9–10 chars like MH12AB1234 or MH12A1234
#     if not (9 <= len(text) <= 10):
#         return False
#     if not (text[0].isalpha() and text[1].isalpha()):
#         return False
#     if not (text[2].isdigit() and text[3].isdigit()):
#         return False
#     if not text[4].isalpha():
#         return False
#     if len(text) == 10 and not text[5].isalpha():
#         return False
#     if len(text) == 9 and not text[5].isdigit():
#         return False
#     if not all(ch.isdigit() for ch in text[-4:]):
#         return False
#     return True

# def clean_and_correct(text: str) -> str:
#     text = re.sub(r'[^A-Z0-9]', '', text.upper())
#     if len(text) > 10:
#         text = text[:10]

#     text_list = list(text)
#     for idx, ch in enumerate(text_list):
#         # letters expected at indices 0,1,4,(5 if len==10); digits elsewhere
#         if idx in [0, 1, 4, 5] and idx < len(text_list):
#             if ch.isdigit():
#                 text_list[idx] = num_to_char.get(ch, ch)
#         else:
#             if ch.isalpha():
#                 text_list[idx] = char_to_num.get(ch, ch)
#     return ''.join(text_list)

# def read_license_plate(lp_img):
#     detections = reader.readtext(lp_img)
#     if not detections:
#         return None, None
#     merged = "".join([det[1] for det in detections]).upper()
#     scores = [det[2] for det in detections if len(det) >= 3]
#     mean_score = sum(scores) / len(scores) if scores else 0.0

#     merged = clean_and_correct(merged)
#     if strict_format(merged):
#         return merged, mean_score
#     return None, None

# def get_car(lp, vehicle_track_ids):
#     x1, y1, x2, y2, score, class_id = lp
#     for xcar1, ycar1, xcar2, ycar2, car_id in vehicle_track_ids:
#         if x1 > xcar1 and y1 > ycar1 and x2 < xcar2 and y2 < ycar2:
#             return xcar1, ycar1, xcar2, ycar2, car_id
#     return -1, -1, -1, -1, -1


# # ---- Levenshtein & similarity
# def _levenshtein(a: str, b: str) -> int:
#     if a == b:
#         return 0
#     if len(a) < len(b):
#         a, b = b, a
#     if len(b) == 0:
#         return len(a)
#     prev = list(range(len(b) + 1))
#     for i, ca in enumerate(a, 1):
#         curr = [i]
#         for j, cb in enumerate(b, 1):
#             ins = prev[j] + 1
#             dele = curr[j - 1] + 1
#             sub = prev[j - 1] + (ca != cb)
#             curr.append(min(ins, dele, sub))
#         prev = curr
#     return prev[-1]

# def _similar_plates(p1: str, p2: str, threshold: float = 0.85) -> bool:
#     if not p1 or not p2:
#         return False
#     if p1 == p2:
#         return True
#     if len(p1) == len(p2) and _levenshtein(p1, p2) <= 1:
#         return True
#     dist = _levenshtein(p1, p2)
#     maxlen = max(len(p1), len(p2))
#     ratio = 1.0 - (dist / maxlen)
#     return ratio >= threshold

# def write_csv(results, output_path, unify_by_plate: bool = True, similarity_threshold: float = 0.85):
#     """
#     CSV columns:
#     frame_nmr, car_id, car_bbox, license_plate_bbox, license_plate_bbox_score, license_number, license_number_score

#     Uses csv.writer + JSON arrays for bboxes to keep the CSV structure robust.
#     """
#     plate_to_gid = {}
#     sort_to_gid = {}
#     next_gid = 0

#     frame_keys = sorted(results.keys(), key=lambda x: int(x) if str(x).isdigit() else x)

#     with open(output_path, 'w', newline='', encoding='utf-8') as f:
#         writer = csv.writer(f)
#         writer.writerow([
#             'frame_nmr',
#             'car_id',
#             'car_bbox',
#             'license_plate_bbox',
#             'license_plate_bbox_score',
#             'license_number',
#             'license_number_score'   # ✅ fixed header
#         ])

#         for frame_nmr in frame_keys:
#             cars = results[frame_nmr]
#             for sort_car_id, data in cars.items():
#                 if 'car' not in data or 'license_plates' not in data:
#                     continue

#                 car_bbox = data['car']['bbox']
#                 lp = data['license_plates']
#                 plate_text = lp.get('text')
#                 license_number_score = lp.get('text_score', 0)  # ✅ renamed for consistency
#                 lp_bbox = lp.get('bbox')
#                 lp_bbox_score = lp.get('bbox_score', 0)

#                 # Determine global ID
#                 if unify_by_plate and plate_text and plate_text != 'None':
#                     assigned_gid = None
#                     for known_plate, gid in plate_to_gid.items():
#                         if _similar_plates(plate_text, known_plate, similarity_threshold):
#                             assigned_gid = gid
#                             break
#                     if assigned_gid is None:
#                         assigned_gid = next_gid
#                         plate_to_gid[plate_text] = assigned_gid
#                         next_gid += 1
#                 else:
#                     if sort_car_id not in sort_to_gid:
#                         sort_to_gid[sort_car_id] = next_gid
#                         next_gid += 1
#                     assigned_gid = sort_to_gid[sort_car_id]

#                 # JSON arrays to avoid CSV parsing issues
#                 car_bbox_json = json.dumps([float(x) for x in car_bbox])
#                 lp_bbox_json = json.dumps([float(x) for x in lp_bbox])

#                 writer.writerow([
#                     int(frame_nmr),
#                     int(assigned_gid),
#                     car_bbox_json,
#                     lp_bbox_json,
#                     float(lp_bbox_score),
#                     plate_text if plate_text is not None else '',
#                     float(license_number_score)   # ✅ now matches header
#                 ])

# # ---- shared bbox parser (robust)
# def parse_bbox(bbox_val):
#     """
#     Convert various bbox formats to [x1, y1, x2, y2] (floats).

#     Accepts:
#       - JSON string like "[x1, y1, x2, y2]"
#       - String with spaces/commas and extra junk, e.g. "[x1 y1 x2 y2]"
#       - Python list/tuple/np.ndarray of length 4
#     """
#     # Already a sequence?
#     if isinstance(bbox_val, (list, tuple, np.ndarray)):
#         if len(bbox_val) == 4:
#             return [float(v) for v in bbox_val]
#         raise ValueError(f"Invalid bbox sequence length: {bbox_val}")

#     # NaN / empty
#     if bbox_val is None:
#         raise ValueError("Empty bbox value")

#     # String?
#     if isinstance(bbox_val, str):
#         s = bbox_val.strip()
#         if not s:
#             raise ValueError("Empty bbox string")

#         # Try JSON parse first
#         try:
#             obj = json.loads(s)
#             if isinstance(obj, (list, tuple)) and len(obj) == 4:
#                 return [float(v) for v in obj]
#         except Exception:
#             pass

#         # Fallback: pull first four numbers via regex (handles stray chars)
#         nums = re.findall(r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?", s)
#         if len(nums) >= 4:
#             return [float(x) for x in nums[:4]]

#         raise ValueError(f"Invalid bbox format: {bbox_val}")

#     # If it’s a plain number, it's malformed data
#     raise ValueError(f"Invalid bbox type/value: {type(bbox_val)} {bbox_val}")



# import re
# import csv
# import json
# import cv2
# import easyocr
# import numpy as np

# # ---------------- OCR setup ----------------
# # Initialize once (set gpu=True if CUDA is available)
# reader = easyocr.Reader(['en'], gpu=False)

# # ---------------- Heuristics for plate cleanup ----------------
# char_to_num = {'O': '0', 'D': '0', 'Q': '0', 'I': '1', 'L': '1', 'Z': '2', 'S': '5', 'B': '8', 'H': 'N'}
# num_to_char = {'0': 'O', '1': 'I', '2': 'Z', '5': 'S', '8': 'B'}

# def strict_format(text: str) -> bool:
#     """
#     Indian plate format: 9–10 chars like MH12AB1234 or MH12A1234.
#     """
#     if not (9 <= len(text) <= 10):
#         return False
#     if not (text[0].isalpha() and text[1].isalpha()):
#         return False
#     if not (text[2].isdigit() and text[3].isdigit()):
#         return False
#     if not text[4].isalpha():
#         return False
#     if len(text) == 10 and not text[5].isalpha():
#         return False
#     if len(text) == 9 and not text[5].isdigit():
#         return False
#     if not all(ch.isdigit() for ch in text[-4:]):
#         return False
#     return True

# def clean_and_correct(text: str) -> str:
#     """
#     Basic cleanup + common confusions mapping.
#     """
#     text = re.sub(r'[^A-Z0-9]', '', text.upper())
#     if len(text) > 10:
#         text = text[:10]

#     text_list = list(text)
#     for idx, ch in enumerate(text_list):
#         # letters expected at indices 0,1,4,(5 if len==10); digits elsewhere
#         if idx in [0, 1, 4, 5] and idx < len(text_list):
#             if ch.isdigit():
#                 text_list[idx] = num_to_char.get(ch, ch)
#         else:
#             if ch.isalpha():
#                 text_list[idx] = char_to_num.get(ch, ch)
#     return ''.join(text_list)

# def read_license_plate(lp_img):
#     """
#     Single-pass OCR: assumes caller already did grayscale + fixed thresholding.
#     We only run EasyOCR once on the provided image and validate/clean.
#     """
#     detections = reader.readtext(lp_img)
#     if not detections:
#         return None, None

#     merged = "".join([det[1] for det in detections]).upper()
#     scores = [det[2] for det in detections if len(det) >= 3]
#     mean_score = float(sum(scores) / len(scores)) if scores else 0.0

#     cleaned = clean_and_correct(merged)
#     if strict_format(cleaned):
#         return cleaned, mean_score
#     return None, None

# # ---- Levenshtein & similarity
# def _levenshtein(a: str, b: str) -> int:
#     if a == b:
#         return 0
#     if len(a) < len(b):
#         a, b = b, a
#     if len(b) == 0:
#         return len(a)
#     prev = list(range(len(b) + 1))
#     for i, ca in enumerate(a, 1):
#         curr = [i]
#         for j, cb in enumerate(b, 1):
#             ins = prev[j] + 1
#             dele = curr[j - 1] + 1
#             sub = prev[j - 1] + (ca != cb)
#             curr.append(min(ins, dele, sub))
#         prev = curr
#     return prev[-1]

# def _similar_plates(p1: str, p2: str, threshold: float = 0.85) -> bool:
#     if not p1 or not p2:
#         return False
#     if p1 == p2:
#         return True
#     if len(p1) == len(p2) and _levenshtein(p1, p2) <= 1:
#         return True
#     dist = _levenshtein(p1, p2)
#     maxlen = max(len(p1), len(p2))
#     ratio = 1.0 - (dist / maxlen)
#     return ratio >= threshold

# def write_csv(results, output_path, unify_by_plate: bool = True, similarity_threshold: float = 0.85):
#     """
#     CSV columns:
#     frame_nmr, car_id, car_bbox, license_plate_bbox, license_plate_bbox_score, license_number, license_number_score

#     Uses csv.writer + JSON arrays for bboxes to keep the CSV structure robust.
#     """
#     plate_to_gid = {}
#     sort_to_gid = {}
#     next_gid = 0

#     frame_keys = sorted(results.keys(), key=lambda x: int(x) if str(x).isdigit() else x)

#     with open(output_path, 'w', newline='', encoding='utf-8') as f:
#         writer = csv.writer(f)
#         writer.writerow([
#             'frame_nmr',
#             'car_id',
#             'car_bbox',
#             'license_plate_bbox',
#             'license_plate_bbox_score',
#             'license_number',
#             'license_number_score'   # header uses this name
#         ])

#         for frame_nmr in frame_keys:
#             cars = results[frame_nmr]
#             for sort_car_id, data in cars.items():
#                 if 'car' not in data or 'license_plates' not in data:
#                     continue

#                 car_bbox = data['car']['bbox']
#                 lp = data['license_plates']
#                 plate_text = lp.get('text')
#                 text_score = lp.get('text_score', 0)  # in-memory key is text_score
#                 lp_bbox = lp.get('bbox')
#                 lp_bbox_score = lp.get('bbox_score', 0)

#                 # Determine global ID
#                 if unify_by_plate and plate_text and plate_text != 'None':
#                     assigned_gid = None
#                     for known_plate, gid in plate_to_gid.items():
#                         if _similar_plates(plate_text, known_plate, similarity_threshold):
#                             assigned_gid = gid
#                             break
#                     if assigned_gid is None:
#                         assigned_gid = next_gid
#                         plate_to_gid[plate_text] = assigned_gid
#                         next_gid += 1
#                 else:
#                     if sort_car_id not in sort_to_gid:
#                         sort_to_gid[sort_car_id] = next_gid
#                         next_gid += 1
#                     assigned_gid = sort_to_gid[sort_car_id]

#                 # JSON arrays to avoid CSV parsing issues
#                 car_bbox_json = json.dumps([float(x) for x in car_bbox])
#                 lp_bbox_json = json.dumps([float(x) for x in lp_bbox])

#                 writer.writerow([
#                     int(frame_nmr),
#                     int(assigned_gid),
#                     car_bbox_json,
#                     lp_bbox_json,
#                     float(lp_bbox_score),
#                     plate_text if plate_text is not None else '',
#                     float(text_score)  # written as license_number_score in CSV
#                 ])

# # ---- shared bbox parser (robust)
# def parse_bbox(bbox_val):
#     """
#     Convert various bbox formats to [x1, y1, x2, y2] (floats).

#     Accepts:
#       - JSON string like "[x1, y1, x2, y2]"
#       - String with spaces/commas and extra junk, e.g. "[x1 y1 x2 y2]"
#       - Python list/tuple/np.ndarray of length 4
#     """
#     # Already a sequence?
#     if isinstance(bbox_val, (list, tuple, np.ndarray)):
#         if len(bbox_val) == 4:
#             return [float(v) for v in bbox_val]
#         raise ValueError(f"Invalid bbox sequence length: {bbox_val}")

#     # NaN / empty
#     if bbox_val is None:
#         raise ValueError("Empty bbox value")

#     # String?
#     if isinstance(bbox_val, str):
#         s = bbox_val.strip()
#         if not s:
#             raise ValueError("Empty bbox string")

#         # Try JSON parse first
#         try:
#             obj = json.loads(s)
#             if isinstance(obj, (list, tuple)) and len(obj) == 4:
#                 return [float(v) for v in obj]
#         except Exception:
#             pass

#         # Fallback: pull first four numbers via regex (handles stray chars)
#         nums = re.findall(r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?", s)
#         if len(nums) >= 4:
#             return [float(x) for x in nums[:4]]

#         raise ValueError(f"Invalid bbox format: {bbox_val}")

#     # If it’s a plain number, it's malformed data
#     raise ValueError(f"Invalid bbox type/value: {type(bbox_val)} {bbox_val}")



# import re
# import csv
# import json
# import cv2
# import easyocr
# import numpy as np

# # ---------------- OCR setup ----------------
# # Keep GPU disabled by default to match most user environments.
# # If you have CUDA and want to enable it, change gpu=False -> gpu=True
# reader = easyocr.Reader(['en'], gpu=False)

# # ---------------- Heuristics for plate cleanup ----------------
# char_to_num = {'O': '0', 'D': '0', 'Q': '0', 'I': '1', 'L': '1', 'Z': '2', 'S': '5', 'B': '8', 'H': 'N'}
# num_to_char = {'0': 'O', '1': 'I', '2': 'Z', '5': 'S', '8': 'B'}


# def strict_format(text: str) -> bool:
#     """
#     Indian plate format: 9–10 chars like MH12AB1234 or MH12A1234.
#     (Kept the strict version you had earlier.)
#     """
#     if text is None:
#         return False
#     if not (9 <= len(text) <= 10):
#         return False
#     if not (text[0].isalpha() and text[1].isalpha()):
#         return False
#     if not (text[2].isdigit() and text[3].isdigit()):
#         return False
#     if not text[4].isalpha():
#         return False
#     if len(text) == 10 and not text[5].isalpha():
#         return False
#     if len(text) == 9 and not text[5].isdigit():
#         return False
#     if not all(ch.isdigit() for ch in text[-4:]):
#         return False
#     return True


# def clean_and_correct(text: str) -> str:
#     """
#     Keep only uppercase A-Z and digits; map common confusions
#     depending on expected position (letters/digits).
#     """
#     if text is None:
#         return ""
#     text = re.sub(r'[^A-Z0-9]', '', text.upper())
#     if len(text) > 10:
#         text = text[:10]

#     text_list = list(text)
#     for idx, ch in enumerate(text_list):
#         # letters expected at indices 0,1,4,(5 if len==10); digits elsewhere
#         if idx in [0, 1, 4, 5] and idx < len(text_list):
#             if ch.isdigit():
#                 text_list[idx] = num_to_char.get(ch, ch)
#         else:
#             if ch.isalpha():
#                 text_list[idx] = char_to_num.get(ch, ch)
#     return ''.join(text_list)




# def read_license_plate(lp_img):
#     """
#     Balanced OCR:
#       - Accepts a color crop (lp_img).
#       - Resizes to a reasonable OCR size.
#       - Runs EasyOCR on three variants:
#           1) raw color crop (often best)
#           2) enhanced (CLAHE -> bilateral -> Otsu threshold)
#           3) adaptive threshold (to recover from uneven lighting)
#       - Cleans results and prefers strict_format results.
#       - Falls back to plausible non-strict results with a small penalty.
#     Returns: (cleaned_plate_or_None, score_or_None)
#     """
#     if lp_img is None:
#         return None, None

#     try:
#         if len(lp_img.shape) == 2:
#             color_raw = cv2.cvtColor(lp_img, cv2.COLOR_GRAY2BGR)
#             gray_base = lp_img
#         else:
#             color_raw = lp_img.copy()
#             gray_base = cv2.cvtColor(color_raw, cv2.COLOR_BGR2GRAY)
#     except Exception:
#         return None, None

#     h, w = gray_base.shape[:2]
#     if w == 0 or h == 0:
#         return None, None

#     target_w = 240
#     scale = target_w / float(w)
#     target_h = max(24, int(h * scale))
#     try:
#         raw_resized = cv2.resize(color_raw, (target_w, target_h), interpolation=cv2.INTER_CUBIC)
#         gray_resized = cv2.cvtColor(raw_resized, cv2.COLOR_BGR2GRAY)
#     except Exception:
#         raw_resized = color_raw
#         gray_resized = gray_base

#     # Variant 1: raw color
#     variants = [("raw", raw_resized)]

#     # Variant 2: enhanced (CLAHE -> bilateral -> Otsu threshold)
#     try:
#         clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8)).apply(gray_resized)
#         bil = cv2.bilateralFilter(clahe, d=9, sigmaColor=75, sigmaSpace=75)
#         _, otsu = cv2.threshold(bil, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
#         variants.append(("enhanced", otsu))
#     except Exception:
#         pass

#     # Variant 3: adaptive threshold (handles uneven lighting)
#     try:
#         adapt = cv2.adaptiveThreshold(
#             gray_resized, 255,
#             cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,
#             11, 2
#         )
#         variants.append(("adaptive", adapt))
#     except Exception:
#         pass

#     best_text = None
#     best_score = -999.0

#     for name, im in variants:
#         try:
#             dets = reader.readtext(im)
#         except Exception:
#             dets = []

#         if not dets:
#             continue

#         merged = "".join([d[1] for d in dets]).upper()
#         scores = [d[2] for d in dets if len(d) >= 3]
#         mean_score = float(sum(scores) / len(scores)) if scores else 0.0

#         cleaned = clean_and_correct(merged)

#         # Scoring priority
#         if strict_format(cleaned):
#             score = mean_score + 0.3
#         else:
#             letters = sum(ch.isalpha() for ch in cleaned)
#             digits = sum(ch.isdigit() for ch in cleaned)
#             plausible = (7 <= len(cleaned) <= 10) and (letters >= 2) and (digits >= 3)
#             if plausible:
#                 score = mean_score - 0.15
#             else:
#                 score = mean_score - 0.5

#         if score > best_score:
#             best_score = score
#             best_text = cleaned

#     if best_text is None:
#         return None, None
#     return best_text, float(best_score)



# def get_car(lp, vehicle_track_ids):
#     """
#     Match a plate detection to a tracked vehicle.
#     lp: (x1,y1,x2,y2,score,class_id) — as produced by YOLO boxes.data.tolist()
#     vehicle_track_ids: iterable of (x1,y1,x2,y2,track_id) from SORT
#     Returns matched car bbox + track_id or (-1,...)
#     """
#     try:
#         x1, y1, x2, y2, score, class_id = lp
#     except Exception:
#         return -1, -1, -1, -1, -1

#     # use plate center containment to match to track boxes (more robust than strict inside)
#     cx = (float(x1) + float(x2)) / 2.0
#     cy = (float(y1) + float(y2)) / 2.0

#     for item in vehicle_track_ids:
#         try:
#             xcar1, ycar1, xcar2, ycar2, car_id = item
#         except Exception:
#             continue
#         if (cx >= xcar1) and (cx <= xcar2) and (cy >= ycar1) and (cy <= ycar2):
#             return int(xcar1), int(ycar1), int(xcar2), int(ycar2), int(car_id)

#     return -1, -1, -1, -1, -1


# # ---- Levenshtein & similarity (unchanged)
# def _levenshtein(a: str, b: str) -> int:
#     if a == b:
#         return 0
#     if len(a) < len(b):
#         a, b = b, a
#     if len(b) == 0:
#         return len(a)
#     prev = list(range(len(b) + 1))
#     for i, ca in enumerate(a, 1):
#         curr = [i]
#         for j, cb in enumerate(b, 1):
#             ins = prev[j] + 1
#             dele = curr[j - 1] + 1
#             sub = prev[j - 1] + (ca != cb)
#             curr.append(min(ins, dele, sub))
#         prev = curr
#     return prev[-1]


# def _similar_plates(p1: str, p2: str, threshold: float = 0.85) -> bool:
#     if not p1 or not p2:
#         return False
#     if p1 == p2:
#         return True
#     if len(p1) == len(p2) and _levenshtein(p1, p2) <= 1:
#         return True
#     dist = _levenshtein(p1, p2)
#     maxlen = max(len(p1), len(p2))
#     ratio = 1.0 - (dist / maxlen)
#     return ratio >= threshold



# def write_csv(results, output_path, unify_by_plate: bool = True, similarity_threshold: float = 0.85):
#     """
#     CSV columns:
#     frame_nmr, car_id, car_bbox, license_plate_bbox, license_plate_bbox_score, license_number, license_number_score

#     Writes only license numbers that pass strict_format().
#     """
#     plate_to_gid = {}
#     sort_to_gid = {}
#     next_gid = 0

#     frame_keys = sorted(results.keys(), key=lambda x: int(x) if str(x).isdigit() else x)

#     with open(output_path, 'w', newline='', encoding='utf-8') as f:
#         writer = csv.writer(f)
#         writer.writerow([
#             'frame_nmr',
#             'car_id',
#             'car_bbox',
#             'license_plate_bbox',
#             'license_plate_bbox_score',
#             'license_number',
#             'license_number_score'
#         ])

#         for frame_nmr in frame_keys:
#             cars = results[frame_nmr]
#             for sort_car_id, data in cars.items():
#                 if 'car' not in data or 'license_plates' not in data:
#                     continue

#                 car_bbox = data['car']['bbox']
#                 lp = data['license_plates']
#                 plate_text = lp.get('text')
#                 text_score = lp.get('text_score', 0)
#                 lp_bbox = lp.get('bbox')
#                 lp_bbox_score = lp.get('bbox_score', 0)

#                 # --- Only keep plate_text if it passes strict_format ---
#                 if plate_text and not strict_format(plate_text):
#                     plate_text = None
#                     text_score = 0.0

#                 # Determine global ID
#                 if unify_by_plate and plate_text:
#                     assigned_gid = None
#                     for known_plate, gid in plate_to_gid.items():
#                         if _similar_plates(plate_text, known_plate, similarity_threshold):
#                             assigned_gid = gid
#                             break
#                     if assigned_gid is None:
#                         assigned_gid = next_gid
#                         plate_to_gid[plate_text] = assigned_gid
#                         next_gid += 1
#                 else:
#                     if sort_car_id not in sort_to_gid:
#                         sort_to_gid[sort_car_id] = next_gid
#                         next_gid += 1
#                     assigned_gid = sort_to_gid[sort_car_id]

#                 car_bbox_json = json.dumps([float(x) for x in car_bbox])
#                 lp_bbox_json = json.dumps([float(x) for x in lp_bbox])

#                 writer.writerow([
#                     int(frame_nmr),
#                     int(assigned_gid),
#                     car_bbox_json,
#                     lp_bbox_json,
#                     float(lp_bbox_score),
#                     plate_text if plate_text else '',
#                     float(text_score)
#                 ])


# # ---- shared bbox parser (robust) - keep from your original util.py
# def parse_bbox(bbox_val):
#     """
#     Convert various bbox formats to [x1, y1, x2, y2] (floats).

#     Accepts:
#       - JSON string like "[x1, y1, x2, y2]"
#       - String with spaces/commas and extra junk, e.g. "[x1 y1 x2 y2]"
#       - Python list/tuple/np.ndarray of length 4
#     """
#     # Already a sequence?
#     if isinstance(bbox_val, (list, tuple, np.ndarray)):
#         if len(bbox_val) == 4:
#             return [float(v) for v in bbox_val]
#         raise ValueError(f"Invalid bbox sequence length: {bbox_val}")

#     # NaN / empty
#     if bbox_val is None:
#         raise ValueError("Empty bbox value")

#     # String?
#     if isinstance(bbox_val, str):
#         s = bbox_val.strip()
#         if not s:
#             raise ValueError("Empty bbox string")

#         # Try JSON parse first
#         try:
#             obj = json.loads(s)
#             if isinstance(obj, (list, tuple)) and len(obj) == 4:
#                 return [float(v) for v in obj]
#         except Exception:
#             pass

#         # Fallback: pull first four numbers via regex (handles stray chars)
#         nums = re.findall(r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?", s)
#         if len(nums) >= 4:
#             return [float(x) for x in nums[:4]]

#         raise ValueError(f"Invalid bbox format: {bbox_val}")

#     # If it’s a plain number, it's malformed data
#     raise ValueError(f"Invalid bbox type/value: {type(bbox_val)} {bbox_val}")




import re
import csv
import json
import cv2
import easyocr
import numpy as np

# ---------------- OCR setup ----------------
# Keep GPU disabled by default to match most user environments.
# If you have CUDA and want to enable it, change gpu=False -> gpu=True
reader = easyocr.Reader(['en'], gpu=False)

# ---------------- Heuristics for plate cleanup ----------------
char_to_num = {'O': '0', 'D': '0', 'Q': '0', 'I': '1', 'L': '1', 'Z': '2', 'S': '5', 'B': '8', 'H': 'N'}
num_to_char = {'0': 'O', '1': 'I', '2': 'Z', '5': 'S', '8': 'B'}


def strict_format(text: str) -> bool:
    """
    Indian plate format: 9–10 chars like MH12AB1234 or MH12A1234.
    """
    if text is None:
        return False
    if not (9 <= len(text) <= 10):
        return False
    if not (text[0].isalpha() and text[1].isalpha()):
        return False
    if not (text[2].isdigit() and text[3].isdigit()):
        return False
    if not text[4].isalpha():
        return False
    if len(text) == 10 and not text[5].isalpha():
        return False
    if len(text) == 9 and not text[5].isdigit():
        return False
    if not all(ch.isdigit() for ch in text[-4:]):
        return False
    return True


def clean_and_correct(text: str) -> str:
    """
    Keep only uppercase A-Z and digits; map common confusions
    depending on expected position (letters/digits).
    """
    if text is None:
        return ""
    text = re.sub(r'[^A-Z0-9]', '', text.upper())
    if len(text) > 10:
        text = text[:10]

    text_list = list(text)
    for idx, ch in enumerate(text_list):
        # letters expected at indices 0,1,4,(5 if len==10); digits elsewhere
        if idx in [0, 1, 4, 5] and idx < len(text_list):
            if ch.isdigit():
                text_list[idx] = num_to_char.get(ch, ch)
        else:
            if ch.isalpha():
                text_list[idx] = char_to_num.get(ch, ch)
    return ''.join(text_list)


def read_license_plate(lp_img):
    """
    Strict-only OCR:
      - Accepts a color crop (lp_img).
      - Resizes to a reasonable OCR size.
    Returns: (best_text, best_score)
    """
    if lp_img is None:
        return None, None

    try:
        if len(lp_img.shape) == 2:
            color_raw = cv2.cvtColor(lp_img, cv2.COLOR_GRAY2BGR)
            gray_base = lp_img
        else:
            color_raw = lp_img.copy()
            gray_base = cv2.cvtColor(color_raw, cv2.COLOR_BGR2GRAY)
    except Exception:
        return None, None

    h, w = gray_base.shape[:2]
    if w == 0 or h == 0:
        return None, None

    target_w = 240
    scale = target_w / float(w)
    target_h = max(24, int(h * scale))
    try:
        raw_resized = cv2.resize(color_raw, (target_w, target_h), interpolation=cv2.INTER_CUBIC)
        gray_resized = cv2.cvtColor(raw_resized, cv2.COLOR_BGR2GRAY)
    except Exception:
        raw_resized = color_raw
        gray_resized = gray_base

    best_text = None
    best_score = -1.0

    dets = reader.readtext(gray_resized)

    merged = "".join([d[1] for d in dets]).upper()
    scores = [d[2] for d in dets if len(d) >= 3]
    mean_score = float(sum(scores) / len(scores)) if scores else 0.0

    cleaned = clean_and_correct(merged)
    if strict_format(cleaned):
        if mean_score > best_score:
            best_score = mean_score
            best_text = cleaned

    if best_text is None:
        return None, None
    return best_text, float(best_score)


def get_car(lp, vehicle_track_ids):
    """
    Match a plate detection to a tracked vehicle.
    lp: (x1,y1,x2,y2,score,class_id) — as produced by YOLO boxes.data.tolist()
    vehicle_track_ids: iterable of (x1,y1,x2,y2,track_id) from SORT
    Returns matched car bbox + track_id or (-1,...)
    """
    try:
        x1, y1, x2, y2, score, class_id = lp
    except Exception:
        return -1, -1, -1, -1, -1

    # Use plate center containment to match to track boxes (more robust than strict inside)
    cx = (float(x1) + float(x2)) / 2.0
    cy = (float(y1) + float(y2)) / 2.0

    for item in vehicle_track_ids:
        try:
            xcar1, ycar1, xcar2, ycar2, car_id = item
        except Exception:
            continue
        if (cx >= xcar1) and (cx <= xcar2) and (cy >= ycar1) and (cy <= ycar2):
            return int(xcar1), int(ycar1), int(xcar2), int(ycar2), int(car_id)

    return -1, -1, -1, -1, -1


# Levenshtein & similarity
def _levenshtein(a: str, b: str) -> int:
    if a == b:
        return 0
    if len(a) < len(b):
        a, b = b, a
    if len(b) == 0:
        return len(a)
    prev = list(range(len(b) + 1))
    for i, ca in enumerate(a, 1):
        curr = [i]
        for j, cb in enumerate(b, 1):
            ins = prev[j] + 1
            dele = curr[j - 1] + 1
            sub = prev[j - 1] + (ca != cb)
            curr.append(min(ins, dele, sub))
        prev = curr
    return prev[-1]


def _similar_plates(p1: str, p2: str, threshold: float = 0.85) -> bool:
    if not p1 or not p2:
        return False
    if p1 == p2:
        return True
    if len(p1) == len(p2) and _levenshtein(p1, p2) <= 1:
        return True
    dist = _levenshtein(p1, p2)
    maxlen = max(len(p1), len(p2))
    ratio = 1.0 - (dist / maxlen)
    return ratio >= threshold


def write_csv(results, output_path, unify_by_plate: bool = True, similarity_threshold: float = 0.85):
    """
    CSV columns:
    frame_nmr, car_id, car_bbox, license_plate_bbox, license_plate_bbox_score, license_number, license_number_score

    Writes only rows whose license_number passes strict_format().
    Returns: number of data rows written (excluding header).
    """
    plate_to_gid = {}
    sort_to_gid = {}
    next_gid = 0
    row_count = 0

    frame_keys = sorted(results.keys(), key=lambda x: int(x) if str(x).isdigit() else x)

    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([
            'frame_nmr',
            'car_id',
            'car_bbox',
            'license_plate_bbox',
            'license_plate_bbox_score',
            'license_number',
            'license_number_score'
        ])

        for frame_nmr in frame_keys:
            cars = results[frame_nmr]
            for sort_car_id, data in cars.items():
                if 'car' not in data or 'license_plates' not in data:
                    continue

                car_bbox = data['car'].get('bbox', [0, 0, 0, 0])
                lp = data['license_plates']
                plate_text = lp.get('text', None)
                text_score = float(lp.get('text_score', 0))
                lp_bbox = lp.get('bbox', [0, 0, 0, 0])
                lp_bbox_score = float(lp.get('bbox_score', 0))

                # Strict-only: skip row if text not valid
                if not plate_text or not strict_format(plate_text):
                    continue

                # Determine global ID (unify identical/near-identical plates)
                if unify_by_plate and plate_text:
                    assigned_gid = None
                    for known_plate, gid in plate_to_gid.items():
                        if _similar_plates(plate_text, known_plate, similarity_threshold):
                            assigned_gid = gid
                            break
                    if assigned_gid is None:
                        assigned_gid = next_gid
                        plate_to_gid[plate_text] = assigned_gid
                        next_gid += 1
                else:
                    if sort_car_id not in sort_to_gid:
                        sort_to_gid[sort_car_id] = next_gid
                        next_gid += 1
                    assigned_gid = sort_to_gid[sort_car_id]

                # JSON arrays to avoid CSV parsing issues
                car_bbox_json = json.dumps([float(x) for x in car_bbox])
                lp_bbox_json = json.dumps([float(x) for x in lp_bbox])

                writer.writerow([
                    int(frame_nmr),
                    int(assigned_gid),
                    car_bbox_json,
                    lp_bbox_json,
                    lp_bbox_score,
                    plate_text,
                    text_score
                ])
                row_count += 1

    return row_count


# ---- shared bbox parser (robust)
def parse_bbox(bbox_val):
    """
    Convert various bbox formats to [x1, y1, x2, y2] (floats).

    Accepts:
      - JSON string like "[x1, y1, x2, y2]"
      - String with spaces/commas and extra junk, e.g. "[x1 y1 x2 y2]"
      - Python list/tuple/np.ndarray of length 4
    """
    # Already a sequence?
    if isinstance(bbox_val, (list, tuple, np.ndarray)):
        if len(bbox_val) == 4:
            return [float(v) for v in bbox_val]
        raise ValueError(f"Invalid bbox sequence length: {bbox_val}")

    # NaN / empty
    if bbox_val is None:
        raise ValueError("Empty bbox value")

    # String?
    if isinstance(bbox_val, str):
        s = bbox_val.strip()
        if not s:
            raise ValueError("Empty bbox string")

        # Try JSON parse first
        try:
            obj = json.loads(s)
            if isinstance(obj, (list, tuple)) and len(obj) == 4:
                return [float(v) for v in obj]
        except Exception:
            pass

        # Fallback: pull first four numbers via regex (handles stray chars)
        nums = re.findall(r"[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?", s)
        if len(nums) >= 4:
            return [float(x) for x in nums[:4]]

        raise ValueError(f"Invalid bbox format: {bbox_val}")

    # If it’s a plain number, it's malformed data
    raise ValueError(f"Invalid bbox type/value: {type(bbox_val)} {bbox_val}")
